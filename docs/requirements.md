...
| **RF-01a** | The email is validated up to **254 characters** and **normalised to lowercase** before being stored and before being queried. Without that, `Ana@x.com` and `ana@x.com` would create two accounts and the unique index would not prevent it. |
...
| — | A body that cannot be read is rejected before any route runs: `400` with `UNREADABLE_BODY` (malformed JSON, unsupported charset) or `413` with `PAYLOAD_TOO_LARGE` and `{ max }` in bytes (over 100 KB). It applies to every endpoint that takes a body, so the endpoint map does not repeat it |
...
router.patch('/:id',
  ...
  validate(updateCaseSchema),  // 3. is the body valid? → 400
